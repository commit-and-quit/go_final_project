...
func ApiTasks(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		w.WriteHeader(http.StatusOK)
		w.Write(ErrToBytes(errors.New("unable to fetch tasks from db")))
	}
...
func ApiTaskPost(req *http.Request) (httpStatus int, resp []byte) {
	var task db.Task
	...
	if task.Title == "" {
		return http.StatusBadRequest, ErrToBytes(errors.New("не указан заголовок задачи"))
	}

	ok, err := misc.CalcDateForDB(&task)
	if !ok {
		return http.StatusInternalServerError, ErrToBytes(err)
	}
...
func ApiTaskPut(req *http.Request) (httpStatus int, resp []byte) {
	var task db.Task
	var buf bytes.Buffer
...
func ApiTask(w http.ResponseWriter, req *http.Request) {
	...
	case http.MethodPost:
		httpCode, resp = ApiTaskPost(req)
	case http.MethodPut:
		httpCode, resp = ApiTaskPut(req)
...
func CheckAuth(next http.HandlerFunc) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(pass) > 0 {
			...
			valid := auth.VerifyUser(cookie.Value)
			if !valid {
				http.Error(w, "Authentification required", http.StatusUnauthorized)
				return
			}
